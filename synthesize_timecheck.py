...
def synthesize(model, vocoder, text, sentence, prefix=''):
    ...
    audio_time = mel_postnet_torch.shape[-1] * 256 / hp.sampling_rate
    return audio_time
...
if __name__ == "__main__":
    ...
    if mode != '4':
        ...
        for s in sentence:
            ...
            audio_time = synthesize(model, vocoder, text, s, prefix='step_{}'.format(args.step))

            synthesize_times.append(time.time() - start_time - preprocess_times[-1])
            audio_times.append(audio_time)            

        print("Inference Time Summary - Real Time Factor(Inference / Audio)")
        for s, pp, syn, aud in zip(sentence, preprocess_times, synthesize_times, audio_times):
            print(f"[{s}]")
            print(
                f"Preprocessing: {pp:.4f}",
                f" - Synthesize: {syn:.4f}",
                f" - Total Inference: {pp + syn:.4f}"
                f" | Audio: {aud:.4f}"
                f" | RTF: {(pp + syn)/aud:.4f}\n"
                )
    else:
        ...
        audio_time = synthesize(model, vocoder, text, sentence, prefix='step_{}'.format(args.step))
        synthesize_time = time.time() - start_time - preprocess_time

        print("Inference Time Summary - Real Time Factor(Inference / Audio)")
        print(f"[{sentence}]")
        print(
            f"Preprocessing: {preprocess_time:.4f}",
            f" - Synthesize: {synthesize_time:.4f}",
            f" - Total Inference: {preprocess_time + synthesize_time:.4f}"
            f" | Audio: {audio_time:.4f}"
            f" | RTF: {(preprocess_time + synthesize_time)/audio_time:.4f}\n"
            )